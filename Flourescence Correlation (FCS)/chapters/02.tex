...
\end{equation}
where $k_B$ is Boltzmann's constant, $T$ is temperature, $\eta$ is viscosity, and $R_h$ is the hydrodynamic radius. Smaller particles diffuse faster (shorter $\tau_D$) while larger particles diffuse slower. Molecular concentration is determined via 

$$c = 1/(G(0) \cdot V_{\text{eff}})$$

where $V_{\text{eff}} = \pi^{3/2} r_0^2 z_0$ is the effective focal volume, enabling quantitative absolute concentration measurements.